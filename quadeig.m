...
if nargout > 2
  ...
  if ((options.pscale == 1 || options.pscale == 2) && tau < toltau)
    ...
    for j = iqz, Y(:,j) = Y(:,j)/norm(Y(:,j)); end
  ...
  end % if ((options.pscale == 1 || options.pscale == 2) && tau < toltau)
...
if nargout > 4
  ...
  for j = 1:2*n
     be_X(j) = norm(Res(:,j))/p(j)/norm(X(:,j));
  end
  be_X = be_X(:);
end
...
if nargout > 5
  ...
  for j = 1:2*n
     be_Y(j) = norm(Res(j,:))/p(j)/norm(Y(:,j));
  end
  be_Y = be_Y(:);
end
