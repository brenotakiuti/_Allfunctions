function [PhiQ_p,PhiQ_n,PhiF_p,PhiF_n,PsiQ_p,PsiQ_n,PsiF_p,PsiF_n,kppa1,kpna1,kepa1,kena1] = eig_order(dofa,D,s,Lx,Phiq,Phia1,Psiq1,Psif1,ee)
...
%     Phifa1(:,ii) = Phiq(dofa+1:2*dofa,ii);
    if abs(real(s(ii)*Lx))<pi/1&abs(imag(s(ii)*Lx))<pi/1
%     if abs(real(s(ii)*Lx))<pi/6&abs(imag(s(ii)*Lx))<pi/6
